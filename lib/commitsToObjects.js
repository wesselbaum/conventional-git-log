...
exports.commitsToObject = function (rawCommits, origin, quiet, replaceSubject, replaceBody, replaceFooter, replaceInterpolated) {
...
${commitBody}`);
  ...
  commitObject.interpolatedCommitMessage = multipleReplace(interpolate(commitTemplate, replacements), replaceInterpolated, quiet);
...
function multipleReplace(string, replaceArrayString, quiet) {
  ...
  try {
    ...
    for (let i = 0; i < replaceArray.length; i++) {
      let regex = new RegExp(replaceArray[i].replace, 'g');

      string = string.replace(regex, replaceArray[i].substring, "\\$&");
    }
    ...
    if (!quiet) {
      console.warn("Failed reading replace " + replaceArray)
    }
    return string;
  }
